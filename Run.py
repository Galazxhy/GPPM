...
def run(args):
    ...
    print('Accuracy:', bestResults[:,2], '\nAverage', sum(bestResults[:,2])/len(bestResults[:,2]), '\nStd', np.std(np.array(bestResults[:,1]), ddof=1))
...
if __name__ == '__main__':
    ...
    # Data
    parser.add_argument('--data', type=str, default='Cora', choices=['Cora', 'Citeseer', 'Pubmed', 'Computers', 'Photo', 'CS', 'Physics'], help='Dataset')
    parser.add_argument('--val_rt', type=float, default='0.2', help='validation ratio of splitting dataset')
